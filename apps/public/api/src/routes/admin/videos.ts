...
export const adminVideosRoute = new Hono<{ Bindings: Env }>();
...
adminVideosRoute.get('/', async (c) => {
  ...
  if (q.q) conditions.push(like(videos.title, `%${q.q}%`));
...
adminVideosRoute.get('/stats', async (c) => {
  ...
  const rows = await db
    .select({ status: videos.aiTaggingStatus, count: sql<number>`count(*)` })
    .from(videos)
