...
export const adminReviewRoute = new Hono();
/** GET /api/admin/review — reviewNeeded=1 の動画一覧 */
adminReviewRoute.get('/', async (c) => {
...
/** PATCH /api/admin/review/:id/correct — 手動タグ修正 */
adminReviewRoute.patch('/:id/correct', async (c) => {
    const db = drizzle(c.env.DB);
    ...
    const update = { reviewNeeded: 0, updatedAt: now };
    if ('map' in body) {
...
/** POST /api/admin/review/:id/reject — 偽陽性として非表示 */
adminReviewRoute.post('/:id/reject', async (c) => {
    const db = drizzle(c.env.DB);
    const id = c.req.param('id');
    await db
        .update(videos)
...
/** POST /api/admin/review/:id/restore — 非表示を復元 */
adminReviewRoute.post('/:id/restore', async (c) => {
    const db = drizzle(c.env.DB);
    const id = c.req.param('id');
    await db
        .update(videos)
...
/** POST /api/admin/review/:id/approve — reviewNeeded=0 にする（値変更なし） */
adminReviewRoute.post('/:id/approve', async (c) => {
    const db = drizzle(c.env.DB);
    const id = c.req.param('id');
    await db
        .update(videos)
