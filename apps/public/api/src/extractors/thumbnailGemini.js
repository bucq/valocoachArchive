import { getAgentPortraitImages, getAllCatalogsLabeled, getCatalogGrids, getMapDisplayImages, getRankCatalogImages, } from './catalogLoader';
import { buildAnalysisPrompt } from './thumbnailShared';
...
export async function analyzeThumbnailGemini(_videoId, imageData, frames, apiKey, title) {
...
async function analyzeThumbnailGeminiInline(catalogs, imageData, frames, apiKey, title) {
    const catalogParts = [{ text: 'これらはリファレンスカタログです。' }];
    let currentCategory = '';
...
export async function analyzeThumbnailGemma(imageData, frames, apiKey, title) {
    // Gemma 3 27B は画像数制限 (32) があるため、個別画像ではなくタイル状のグリッド画像を使用する
    const grids = getCatalogGrids();
    const catalogParts = [{ text: 'リファレンスカタログ（グリッド一覧）:' }];
 ...
 */
export async function analyzeThumbnailGemmaStepwise(imageData, frames, apiKey, title) {
    ...
    if (!mapResult && !agentResult && !rankResult)
        return null;
    return {
...
async function inferStepMap(minimapFrames, apiKey) {
    const mapImages = getMapDisplayImages();
    const parts = [{ text: 'マップミニマップカタログ（ゲーム左上表示と比較用）:' }];
...
async function inferStepAgent(imageData, apiKey, title) {
    const agentImages = getAgentPortraitImages();
    const parts = [{ text: 'エージェント全身像カタログ:' }];
...
async function inferStepRank(imageData, fullFrames, apiKey) {
    const rankImages = getRankCatalogImages();
    const parts = [{ text: 'ランクアイコンカタログ（IRON_1〜RADIANT）:' }];
...
async function callGeminiStepApi(url, apiKey, body, step) {
    try {
        ...
        try {
            return parseStepResult(JSON.parse(text));
        }
        catch {
            const sanitized = sanitizeJsonStrings(text);
            try {
                return parseStepResult(JSON.parse(sanitized));
            }
            catch (err) {
...
export async function callGeminiApi(url, apiKey, body) {
    try {
        const res = await fetch(url, {
            method: 'POST',
            headers: {
        ...
        try {
            ...
            const sanitized = sanitizeJsonStrings(text);
            try {
                return normalizeGeminiResult(JSON.parse(sanitized));
            }
            catch (parseErr) {
 ...
 * Gemini 用（parts 配列）。
 */
function buildPerRequestParts(imageData, frames, title) {
    const parts = [];
...
function normalizeGeminiResult(raw) {
    const validConf = (v) => v === 'high' || v === 'medium' || v === 'low' ? v : 'low';
    return {
        ...raw,
