 ...
 */
export async function fetchChannelVideos(channelId, apiKey, maxResults = 9999) {
    // Step 1: uploads プレイリスト ID を取得（1 unit）
    const chRes = await fetch(`${YT_BASE}/channels?${new URLSearchParams({ part: 'contentDetails', id: channelId, key: apiKey })}`);
    if (!chRes.ok)
 ...
 */
export async function searchVideos(query, apiKey, maxResults = 50) {
    const params = new URLSearchParams({
        part: 'id',
        q: query,
        type: 'video',
    ...
    const vRes = await fetch(`${YT_BASE}/videos?part=snippet,contentDetails,statistics&id=${ids}&key=${apiKey}`);
    if (!vRes.ok)
