...
export interface TagSingleOptions {
  geminiApiKey: string;
...
export async function getPendingVideos(db: Db, maxCount = 500): Promise<TagPendingVideoRow[]> {
  return db
    .select({
      id: videos.id,
      title: videos.title,
...
export async function tagSingleVideo(
  db: Db,
  video: TagPendingVideoRow,
  opts: TagSingleOptions,
): Promise<TagSingleResult> {
...
/** タグ付けステータスをリセット */
export async function resetTaggingStatus(db: Db, statuses: string[]): Promise<number> {
  const now = new Date().toISOString();
