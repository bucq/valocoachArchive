...
export default function VideoCard({ video, onClick, onReport }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* サムネイル */}
      <div style={{ position: 'relative', aspectRatio: '16/9', background: 'var(--c-surface-2)', overflow: 'hidden' }}>
        <img
      ...
      <div style={{ padding: '12px 14px 14px' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', alignItems: 'center' }}>
          {video.map && video.map !== 'null' && (
            <span className="map-badge">{video.map}</span>
          )}
          {video.agent && video.agent !== 'null' && (
            <span className="agent-badge" style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
          ...
          {video.rank && video.rank !== 'null' && rankClass && (
            <span className={`rank-badge rank-${rankClass}`} style={{ display: 'inline-flex', alignItems: 'center', gap: '3px' }}>
          ...
          <button
            onClick={e => { e.stopPropagation(); onReport(video); }}
            title="タグを修正提案する"
            style={{
              marginLeft: 'auto',
            ...
            onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.background = 'rgba(255,70,85,0.2)'; el.style.borderColor = '#FF4655'; }}
            onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.background = 'rgba(255,70,85,0.08)'; el.style.borderColor = 'rgba(255,70,85,0.25)'; }}
          >
