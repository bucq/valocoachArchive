...
import { MAP_ICONS, AGENT_ICONS, RANK_ICONS } from '../lib/valorant-assets';
...
function IconSelect({ label, value, options, icons, iconClip, iconSize = 16, onChange }: IconSelectProps) {
          ...
          <div style={{
            ...
            <button
              ...
              style={{
                ...
                color: '#555', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.72rem',
                textAlign: 'left',
              }}
            >
...
export default function CorrectionModal({
  apiBase, videoId, title,
  currentMap, currentAgent, currentRank,
  onClose,
}: Props) {
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            color: '#666', fontSize: '1.1rem', lineHeight: 1,
          }}
        >
          ×
        </button>
        ...
        <div style={{ marginBottom: '1.25rem' }}>
          <div style={{
            ...
            color: '#FF4655', marginBottom: '0.4rem', textTransform: 'uppercase',
          }}>
            // タグ修正リクエスト
          </div>
          <p style={{
        ...
        <div style={{
          ...
          <div style={{ color: '#555', marginBottom: '0.3rem', letterSpacing: '0.1em' }}>現在のタグ</div>
          <div style={{ color: '#888', display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
            <span>map: <span style={{ color: '#aaa' }}>{currentMap ?? '—'}</span></span>
            <span>agent: <span style={{ color: '#aaa' }}>{currentAgent ?? '—'}</span></span>
            <span>rank: <span style={{ color: '#aaa' }}>{currentRank ?? '—'}</span></span>
          </div>
        </div>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: selected ? '0 0 0 1px rgba(255,70,85,0.5), 0 2px 10px rgba(255,70,85,0.35)' : 'none',
                    }}
                  >
                    <div style={{
        ...
        {/* Status messages */}
        {status === 'success' && (
          <div style={{ color: '#45D483', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.72rem', marginBottom: '0.75rem' }}>
            ✔ 修正リクエストを送信しました
          </div>
        )}
        {status === 'alreadyPending' && (
          <div style={{ color: '#F5C842', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.72rem', marginBottom: '0.75rem' }}>
            ⚠ 他のユーザーからすでにリクエストが届いています。管理者が対応中です。
          </div>
        )}
        {status === 'error' && (
          <div style={{ color: '#FF4655', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.72rem', marginBottom: '0.75rem' }}>
        ...
        {status !== 'success' && (
          <button
            ...
            style={{
              ...
              fontFamily: 'JetBrains Mono, monospace', fontSize: '0.75rem',
              letterSpacing: '0.1em', cursor: hasChange ? 'pointer' : 'not-allowed',
